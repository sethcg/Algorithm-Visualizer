...
const quickSortHelper = (boxes: Box[], leftIndex: number, rightIndex: number, steps: Box[][]) => {
...
const partition = (boxes: Box[], leftIndex: number, rightIndex: number, steps: Box[][]) => {
    ...
    let i: number = (leftIndex - 1), j: number, temp: Box;
